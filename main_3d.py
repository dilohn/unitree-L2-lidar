...
def add_points(points):
    ...
    with points_lock:
        ...
        if PERSISTENCE > 0:
            new_timestamps = [current_time] * len(new_cloud.points)
            point_timestamps.extend(new_timestamps)
...
def remove_old_points():
    ...
    if PERSISTENCE <= 0 or len(point_timestamps) == 0:
        return False
    
    current_time = time.time()
    cutoff_time = current_time - PERSISTENCE
    
    keep_indices = [i for i, ts in enumerate(point_timestamps) if ts >= cutoff_time]
    ...
    new_points = points_array[keep_indices]
    new_colors = colors_array[keep_indices]
    new_timestamps = [point_timestamps[i] for i in keep_indices]
    
    point_cloud.points = o3d.utility.Vector3dVector(new_points)
    point_cloud.colors = o3d.utility.Vector3dVector(new_colors)
    point_timestamps = new_timestamps
    
    removed_count = len(points_array) - len(new_points)
    if removed_count > 0:
        print(f"Removed {removed_count} old points (persistence: {PERSISTENCE}s)")
...
def visualization_thread():
    ...
    window_title = "LiDAR Point Cloud Visualization"
    if PERSISTENCE > 0:
        window_title += f" (Persistence: {PERSISTENCE}s)"
    vis.create_window(window_title, width=1024, height=768)
    ...
    while running:
        ...
        if PERSISTENCE > 0 and (current_time - last_persistence_check) > UPDATE_INTERVAL:
            with points_lock:
                if remove_old_points():
                    update_needed = True
            last_persistence_check = current_time
        
        if new_points_available and (current_time - last_update_time) > UPDATE_INTERVAL:
            with points_lock:
